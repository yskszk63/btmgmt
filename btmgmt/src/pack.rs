...
macro_rules! packable_flags {
    (
        $(#[$attrs:meta])*
        pub struct $module:ident :: $name:ident : $ty:ty {
            $(
                $(#[$fattrs:meta])*
                const $fname:ident = $bit:literal;
            )+
        }
    ) => {
        pub use $module :: $name;
        ...
        impl $crate::pack::Unpack for $name {
            fn unpack<B: bytes::Buf>(buf: &mut B) -> $crate::pack::Result<Self> {
                Ok(Self::from_bits_truncate($crate::pack::Unpack::unpack(buf)?))
            }
        }
    }
}
